...
public class FileEngine {
   ...
   private final String FilePath; // Making FilePath immutable
    ...
    public void CreateEntry(String Key, JSONObject Val, int TimeToLive) throws Exception // Create method, if TimeToLive is provided
    {    
         try {if (Key.length()>32) // Check if Key is more than 32 Char
        	 throw new KeySizeExceeded();
         else if((instrument.getObjectSize((Object)Val)/1024)>16) // Check if JSONObject is more than 16 KB
         ...
         try (FileReader reader = new FileReader(FilePath)) {
            ...
            int Stamp = t.toSecondOfDay();
            tempArray.put(Stamp);  // Adding time of creation of Key as third element
    ...
    public void CreateEntry(String Key, JSONObject Val) throws Exception // Create method , if no TimeToLive is specified by user
    {
    	try {if (Key.length()>32) // Check if Key is more than 32 Char
       	 throw new KeySizeExceeded();
    	 else if((instrument.getObjectSize((Object)Val)/1024)>16) // Check if JSONObject is more than 16 KB
        ...
        try (FileReader reader = new FileReader(FilePath)) {
            ...
            LocalTime t = LocalTime.now();
            int Stamp = t.toSecondOfDay();
            tempArray.put(Stamp); //Adding time stamp to array
            temp.put(Key, tempArray);
    ...
    public JSONObject ReadEntry(String Key) throws Exception //Read method , which returns JSONObject
    {
        ...
        try (FileReader reader = new FileReader(FilePath)) {
            ...
            if (temp.has(Key)) //Check if JSONObject has the given key value pair
            {
                ...
                if ((Curr - tempArray.getInt(2)) < tempArray.getInt(1)) //Checking time to live condition
